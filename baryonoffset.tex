...
%With two offset grids of different masses this symmetry is broken and the particle grid forces are anisotropic. The force on a particle from the baryon grid thus operates in a different direction than the force on a particle from the CDM grid, reducing the force between the species baryons to CDM. In other words, there is a spurious growth term proportional to both the phase-angle between baryons and CDM and the mass difference.

%\YF{$d_{bNL} = d_{b} + ... + (\alpha d_b + \beta d_{cdm}) <d_{b}, d_{cdm}> + ...$ $\alpha, \beta$ does depend on the type of gravity solver and mass difference.}
